<?php
class SatellitePaginate extends SatellitePlugin {
	...
	function SatellitePaginate($table = '', $fields = '', $sub = '', $parent = '') {
		$this -> sub = $sub;
		$this -> parentd = $parent;
	...
	function start_paging($page = '') {
		...
		if (count($records) < $allRecordsCount) {			
			...
			$this -> pagination .= '<span class="displaying-num">' . __('Displaying', SATL_PLUGIN_NAME) . ' ' . ($this -> begRecord + 1) . ' - ' . ($this -> begRecord + count($records)) . ' ' . __('of', SATL_PLUGIN_NAME) . ' ' . $allRecordsCount . '</span>';
		
			if ($this -> page > 1) {
				$this -> pagination .= '<a class="prev page-numbers" href="' . SatelliteHtmlHelper::retainquery($add_prev) . '" title="' . __('Previous Page', SATL_PLUGIN_NAME) . '">&laquo;</a>';
			}
			...
			while ($p <= $allRecordsCount) {
				...
				if ($k >= ($this -> page - 5) && $k <= ($this -> page + 5)) {
					if ($k != $this -> page) {
						$this -> pagination .= '<a class="page-numbers" href="' . SatelliteHtmlHelper::retainquery($add_numbers) . '" title="' . __('Page', SATL_PLUGIN_NAME) . ' ' . $k . '">' . $k . '</a>';
					} else {
			...
			if ((count($records) + $this -> begRecord) < $allRecordsCount) {
				$this -> pagination .= '<a class="next page-numbers" href="' . SatelliteHtmlHelper::retainquery($add_next) . '" title="' . __('Next Page', SATL_PLUGIN_NAME) . '">&raquo;</a>';
			}
		}
