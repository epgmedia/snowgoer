...
?>
	...
	<?php 
		if (($sidebarname == "Home Sidebar") || ($sidebarname == "Non-Home Sidebar") || ($sidebarname == "Sports Center Sidebar")) { $sidebarsize = "300px wide by 250px tall"; $adwidth=get_option('home_right_column'); } 
		if ($sidebarname == "Home Bottom Narrow") { $sidebarsize = "160px wide by 600px tall"; $adwidth=get_option('home_narrow_column'); } 
		if ($sidebarname == "Home Bottom Wide") { $sidebarsize = "custom 400px wide"; $adwidth=get_option('home_wide_column'); } 
		if ($sidebarname == "Home Main Column") { $sidebarsize = "custom 590px wide"; $adwidth=get_option('home_full_width_column'); } 
		if (($sidebarname == "Home Bottom Left") || ($sidebarname == "Home Bottom Right")) { $sidebarsize = "custom 280px wide"; $adwidth=get_option('home_left_column'); } 
		if ($sidebarsize) { $instance['sidebarsize'] = $sidebarsize; } 
		if ($adwidth) { $instance['adwidth'] = $adwidth; } 
...
<p>If you want the advertisement to link to another Website, add a link in the box below.</p>
		...
		<input type="hidden" id="<?php echo $this->get_field_id('sidebarsize'); ?>" name="<?php echo $this->get_field_name('sidebarsize'); ?>" value="<?php echo $instance['sidebarsize']; ?>" />
		<input type="hidden" id="<?php echo $this->get_field_id('adwidth'); ?>" name="<?php echo $this->get_field_name('adwidth'); ?>" value="<?php echo $instance['adwidth']; ?>" />
