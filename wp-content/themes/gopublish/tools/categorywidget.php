...
class sno_category extends WP_Widget {
	...
	function sno_category() {
		$widget_ops  = array(
			'classname'   => 'GoPublish Category Widget',
			'description' => 'Use this widget to add a Category Display box.'
		);
		...
		if ( ( $instance['category'] == - 1 ) || ( $totalstories <= 0 ) ) {
			...
			if ( have_posts() ) : while ( have_posts() ) : the_post();
				...
				if ( $count <= $instance['number'] ) {
					...
					if ( ( $instance['category-photo-size'] == "Large" )
					     && ( $instance['sidebarname'] == "Home Bottom Wide" )
					) {
						$photodisplaywidth = $instance['photowidth'];
						$thumbnailsize     = "home400";
					} else if ( ( $instance['category-photo-size'] == "Large" )
					            && ( $instance['sidebarname'] == "Home Main Column" )
					) {
						$photodisplaywidth = $instance['photowidth'];
						$thumbnailsize     = "permalink";
					              ...
					              && ( $instance['sidebarname'] == "Home Bottom Right" ) )
					            || ( ( $instance['category-photo-size'] == "Large" )
					                 && ( $instance['sidebarname'] == "Home Bottom Left" ) )
					) {
						$photodisplaywidth = $instance['photowidth'];
						...
						$thumbnailsize     = "home160";
					            ...
					            || ( ( $instance['category-photo-size'] == "Large" )
					                 && ( $instance['sidebarname'] == "Non-Home Sidebar" ) )
					            || ( ( $instance['category-photo-size'] == "Large" )
					                 && ( $instance['sidebarname']
					                      == "Sports Center Sidebar" ) )
					) {
						$photodisplaywidth = $instance['photowidth'];
						$thumbnailsize     = "permalink";
					}

					if ( ( $instance['category-photo-size'] == "Small" )
					     && ( $instance['sidebarname'] == "Home Bottom Wide" )
					) {
						$photodisplaywidth = get_option( 'home_narrow_column' );
						$thumbnailsize     = "home160";
					} else if ( ( $instance['category-photo-size'] == "Small" )
					            && ( $instance['sidebarname'] == "Home Main Column" )
					) {
						$photodisplaywidth = get_option( 'home_narrow_column' );
						$thumbnailsize     = "home160";
						...
						$photodisplaywidth = 120;
						$thumbnailsize     = "home120";
					}

					if ( ( $instance['category-photo-size'] == "Medium" )
					     && ( $instance['sidebarname'] == "Home Bottom Wide" )
					) {
						$photodisplaywidth = 200;
						$thumbnailsize     = "archive";
					} else if ( ( $instance['category-photo-size'] == "Medium" )
					            && ( $instance['sidebarname'] == "Home Main Column" )
					) {
						$photodisplaywidth = 200;
						$thumbnailsize     = "archive";
					...
					if ( $photodisplaywidth == "" ) {
						$photodisplaywidth = get_option( 'home_narrow_column' );
						$thumbnailsize     = "home160";
					}

					if ( ( $photodisplaywidth == get_option( 'home_right_column' ) )
					     && ( $instance['sidebarname'] == "Home Sidebar" )
					) {
						$marginkey = 3;
					}
					if ( ( $photodisplaywidth == get_option( 'home_narrow_column' ) )
					     && ( $instance['sidebarname'] == "Home Bottom Narrow" )
					) {
						$marginkey = 3;
					}
					if ( ( $photodisplaywidth == get_option( 'non_home_right_column' ) )
					     && ( $instance['sidebarname'] == "Non-Home Sidebar" )
					) {
						$marginkey = 3;
					}
					...
					if ( ( $photodisplaywidth == get_option( 'home_left_column' ) )
					     && ( $instance['sidebarname'] == "Home Bottom Left" )
					) {
						$marginkey = 3;
					}
					if ( ( $photodisplaywidth == get_option( 'home_center_column' ) )
					     && ( $instance['sidebarname'] == "Home Bottom Right" )
					) {
						$marginkey = 3;
					}
					if ( ( $photodisplaywidth == get_option( 'home_wide_column' ) )
					     && ( $instance['sidebarname'] == "Home Bottom Wide" )
					) {
						$marginkey = 3;
					}
					...
					<?php if ( ( $instance['category-photo-size'] == "Large" )
					           && ( has_post_thumbnail() )
					) {
						...
						<?php if ( $instance['teaser-thumb'] == on ) {
							...
							if ( has_post_thumbnail() ) {
								...
								?><a href="<?php the_permalink(); ?>"><img
									...
									style="float:<?php echo $thumbplacement; ?>;margin-bottom:10px;margin-<?php echo $thumbplacement; ?>:0px;width:70px"/>
								</a><?php
							} else {
								global $wpdb;
								$attachment_id =
									$wpdb->get_var( "SELECT ID FROM $wpdb->posts WHERE post_parent = '$post->ID' AND post_status = 'inherit' AND post_type='attachment' ORDER BY post_date DESC LIMIT 1" );
								$attachment    = wp_get_attachment_url( $attachment_id );
								if ( $attachment ) {
									?><a href="<?php the_permalink(); ?>"><img
										src="<?php echo $attachment; ?>" class="catboxthumb"
										style="float:<?php echo $thumbplacement; ?>;margin-bottom:10px;margin-<?php echo $thumbplacement; ?>:0px;width:70px"/>
									</a><?php }
							} ?>
			...
			if ( $instance['view-all'] == on ) {
				...
				<a href="<?php echo cat_id_to_slug( $instance['category'] ); ?>"><p
						class="viewall">View All <span class="arrows"> &raquo;</span></p></a>
			<?php } ?>
	...
	function form( $instance ) {
		$defaults = array(
			...
			'number-headlines'         => '3',
			'headline-size'            => '18',
			'category-photo-placement' => 'Left',
			'category-photo-size'      => 'Medium',
		...
		<div
			...
			<p>
				...
				<label
					for="<?php echo $this->get_field_id( 'number' ); ?>"><?php _e( 'Number of Stories' ); ?></label>
				<br/>
				<select id="<?php echo $this->get_field_id( 'category-photo-placement' ); ?>"
				        name="<?php echo $this->get_field_name( 'category-photo-placement' ); ?>">
					<option value="Left" <?php if ( 'Left'
					                                == $instance['category-photo-placement']
					) {
				...
				<select id="<?php echo $this->get_field_id( 'category-photo-size' ); ?>"
					...
					<option value="Small" <?php if ( 'Small'
					                                 == $instance['category-photo-size']
					) {
						echo 'selected="selected"';
					} ?>>Small
					</option>
					<option value="Medium" <?php if ( 'Medium'
					                                  == $instance['category-photo-size']
					) {
						echo 'selected="selected"';
					} ?>>Medium
					</option>
					<option value="Large" <?php if ( 'Large'
					                                 == $instance['category-photo-size']
					) {
						echo 'selected="selected"';
					} ?>>Large
			...
			<p>
				...
				<label
					for="<?php echo $this->get_field_id( 'number-headlines' ); ?>"><?php _e( 'Number of Extra Headlines' ); ?></label><br/>
				<input id="<?php echo $this->get_field_id( 'headline-teaser' ); ?>"
				       name="<?php echo $this->get_field_name( 'headline-teaser' ); ?>"
				       type="text" maxlength="3" size="3"
